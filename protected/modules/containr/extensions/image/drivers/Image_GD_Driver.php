...
class Image_GD_Driver extends Image_Driver {
	...
	public function addlayer($properties) {

		// Set dimensions of new image
		$width = $properties['width'];
		$height = $properties['height'];

		$widthOrig = imagesx($this->tmp_image);
		$heightOrig = imagesy($this->tmp_image);

		// Set colors
		$colorR = $properties['color'][0];
		$colorG = $properties['color'][1];
		$colorB = $properties['color'][2];

		// Set background color
		$background = imagecreatetruecolor($width,$height);
		$colorFill = imagecolorallocate($background, $colorR, $colorG, $colorB);
		
		// Fill image with background color
		imagefill($background,0,0,$colorFill);

		// Copy original image into new image
		if($status = imagecopyresampled($background, $this->tmp_image,($width-$widthOrig)/2,($height-$heightOrig)/2, 0, 0, $widthOrig, $heightOrig, $widthOrig, $heightOrig)) {

			imagedestroy($this->tmp_image);
			$this->tmp_image = $background;
		}
	...
	public function watermark($properties)
	{
		...
		$white = imagecolorallocate($this->tmp_image, 0, 0, 0);
		$grey = imagecolorallocate($this->tmp_image, 128, 128, 128);
		$black = imagecolorallocate($this->tmp_image, 255, 255, 255);

		// Execute the watermarking
		imagettftext($this->tmp_image, $properties['fontsize'], 90, $width-10, $height-10, $white, $_SERVER["DOCUMENT_ROOT"] . $properties['font'], $properties['text']);
		$status = imagettftext($this->tmp_image, $properties['fontsize'], 90, $width-11, $height-11, $black, $_SERVER["DOCUMENT_ROOT"] . $properties['font'], $properties['text']);
