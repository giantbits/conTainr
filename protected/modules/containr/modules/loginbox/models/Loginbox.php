<?php
class Loginbox extends CActiveRecord {
	public $pagelist = '';
	public $elemType = 'loginbox';

	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return ContentImage the static model class
	 */
	public static function model($className=__CLASS__)
	{
		return parent::model($className);
	}

	public function behaviors() {
		return array(
			'ElementBehavior'=>array(
				'class'=>'application.modules.containr.components.ContainrElementBehavior',
				'templates'=>array('maincontent'=>'Main content','relatedcontent'=>'Related content')
			)
		);
	}

	/**
	 * @return string the associated database table name
	 */
	public function tableName()
	{
		return 'containr_loginbox';
	}

	/**
	 * @return array validation rules for model attributes.
	 */
	public function rules()
	{
		// NOTE: you should only define rules for those attributes that
		// will receive user inputs.
		return array(
			array('showHeadline,adminActivation,redirectAfterLogin,enableRegistration,doubleoptin', 'numerical', 'integerOnly'=>true),
			array('title', 'length', 'max'=>255),
			array('showHeadline,adminActivation,redirectAfterLogin,enableRegistration,doubleoptin', 'safe'),
			array('title,redirectAfterLogin','required'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, title', 'safe', 'on'=>'search'),
		);
	}

	/**
	 * @return array relational rules.
	 */
	public function relations()
	{
		// NOTE: you may need to adjust the relation name and the related
		// class name for the relations automatically generated below.
		return array(
		);
	}

	/**
	 * @return array customized attribute labels (name=>label)
	 */
	public function attributeLabels()
	{
		return array(
			'id' => 'ID',
			'title' => 'Title',
			'showHeadline' => 'Display title',
			'adminActivation' => 'Admin activation required',
			'redirectAfterLogin' => 'Redirect after Login',
			'enableRegistration' => 'Enable Registration',
			'doubleoptin' => 'Double Opt-In',
		);
	}

	public function buildPageList()
	{
		$pageList = '';

		$libId = ContainrElementLib::model()->find('elementId = ' . $this->id);

		$refLib = ContainrElementPageRef::model()->findAll('elementId = ' . $libId->id);

		foreach($refLib as $ref)
		{
			$page = ContainrPage::model()->findByPK($ref->pageId);
			$pageList .= CHtml::Link($page->title,Yii::app()->createUrl('/containr/page/update/',array('id'=>$page->id)));
		}

		$this->pagelist = $pageList;
	}

	/**
	 * Retrieves a list of models based on the current search/filter conditions.
	 * @return CActiveDataProvider the data provider that can return the models based on the search/filter conditions.
	 */
	public function search()
	{
		// Warning: Please modify the following code to remove attributes that
		// should not be searched.

		$criteria = new CDbCriteria;

		$criteria->compare('id',$this->id);
		$criteria->compare('title',$this->title,true);
		$criteria->compare('showHeadline',$this->showHeadline);
		$criteria->compare('adminActivation',$this->showHeadline);
		$criteria->compare('redirectAfterLogin',$this->showHeadline);
		$criteria->compare('enableRegistration',$this->showHeadline);
		$criteria->compare('doubleoptin',$this->showHeadline);
		return new CActiveDataProvider(get_class($this), array(
			'criteria'=>$criteria,
		));
	}

	public function afterFind()
	{
		parent::afterFind();

		$this->buildPageList();
	}

	public function beforeDelete(){


	}

}
?>
