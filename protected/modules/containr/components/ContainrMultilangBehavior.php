...
class ContainrMultilangBehavior extends CActiveRecordBehavior
{
    /**
    ...
    private $_istranslationsubmit = false;

    private $_translationSubmitData = array();
    ...
    public function attach($owner) {
        ...
        // fetch languages from ContainrUrlManager
        ...
        if (!isset($this->localizedRelation)) {
            $this->localizedRelation = 'i18n' . str_replace("containr_", "",$owner_classname);
        }
        ...
        if (!isset($this->langForeignKey)) {
            $this->langForeignKey = str_replace("containr_", "", $simple_table_name) . '_id';
        }
    ...
    public function beforeSave( $event ) {
        ...
        if(isset($_GET['clkey']) && $_GET['clkey'] <> $this->defaultLanguage) {
        	$this->_istranslationsubmit = true;

        	foreach($this->localizedAttributes as $field) {
        		$this->_translationSubmitData[$field] = $main_owner->$field;
        		unset($main_owner->$field);
        	}
        }
    }
    ...
    public function afterSave($event) {
    	...
    	$lang = (isset($_GET['clkey'])) ? $_GET['clkey'] : $this->defaultLanguage;

    	$main_owner = $this->getOwner();
        $ownerPk = $main_owner->getPrimaryKey();
        $rs = array();

        if($this->_istranslationsubmit) {

            if (!$main_owner->isNewRecord) {
                $model = call_user_func(array($this->langClassName, 'model'));
                $c = new CdbCriteria();
                $c->condition = "{$this->langForeignKey}=:id AND {$this->langField}=:lang";
                $c->params = array('id'=>$ownerPk,'lang'=>$lang);
                $c->index = $this->langField;
                $rs = $model->findAll($c);
            }

            // new translation entry or update of existing one
            if (!isset($rs[$lang])) {
                $owner = new $this->langClassName;
            ...
            foreach ($this->localizedAttributes as $field) {
                ...
                if(!$this->_istranslationsubmit) {
                    ...
                    $value = $this->_translationSubmitData[$field]; //$this->getLangAttribute($field . '_'.$lang);
                }
