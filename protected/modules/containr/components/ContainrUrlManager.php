 ...
 * @version 1.0
 */
class ContainrUrlManager extends CUrlManager
{
    /**
    ...
    public function addLanguageUrlRules()
    {
        if (!$this->_languagesRuleAdded && $this->languagesEnabled())
        {
            ...
            $rules[$langRoute] = '/';
            $rules[$langRoute . '/'] = '/';
            //$rules[$langRoute . '/containr/<module:\w+>/<controller:\w+>/<action:\w+>'] = '/containr/<module>/<controller>/<action>';

            $rules[$langRoute . '/containr/<module:\w+>/<controller:\w+>/<id:\d+>'] = '/containr/<module>/<controller>/view';
            $rules[$langRoute . '/containr/<module:\w+>/<controller:\w+>/<action:\w+>/<id:\d+>'] = '/containr/<module>/<controller>/<action>';
            $rules[$langRoute . '/containr/<module:\w+>/<controller:\w+>/<action:\w+>'] = '/containr/<module>/<controller>/<action>';
            $rules[$langRoute . '/containr/<module:\w+>/<controller:\w+>'] = '/containr/<module>/<controller>';

            $rules[$langRoute . '/<module:\w+>/<controller:\w+>/<id:\d+>'] = '<module>/<controller>/view';
            $rules[$langRoute . '/<module:\w+>/<controller:\w+>/<action:\w+>/<id:\d+>'] = '<module>/<controller>/<action>';
            $rules[$langRoute . '/<module:\w+>/<controller:\w+>/<action:\w+>'] = '<module>/<controller>/<action>';
            $rules[$langRoute . '/<module:\w+>/<controller:\w+>'] = '<module>/<controller>';


            $rules[$langRoute . '/<code:[\w\/]+>'] = 'site/index';
            $rules[$langRoute . '/<route:[\w\/]+>'] = '<route>';
            //$rules[$langRoute . '/containr/<m:\w+>/<c:\w+>/<a:\w+>'] = '<m>/<c>/<a>';
    ...
    public function createUrl($route, $params = array(), $ampersand = '&')
    {
        ...
        if (substr($route,0,5) == 'page:') {
            ...
            if ($page != null) {
                ...
                return rtrim(Yii::app()->createUrl(implode('/',$url),$params),'/');
            }
        }
    ...
    public function parseUrl($request)
    {
        ...
        if (substr($request->pathInfo,0,9)=='containr/') {
            return parent::parseUrl($request);
        }

        $cache = $this->getCachedPage($request->pathInfo);
