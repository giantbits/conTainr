...
Yii::import('application.modules.containr.extensions.google.Google_Client', true);
Yii::import('application.modules.containr.extensions.google.contrib.Google_AnalyticsService', true);

class ContainrAnalytics extends CComponent
{
	public $client;
	protected $analytics;
	...
	public function __construct(array $config=array())
	{
		...
		try {
			...
			throw new CHttpException(500,'There was an Google Analytics API service error');
			 //{code}: {message}',array('{code}'=>$e->getCode(),'{message}'=>$e->getMessage())));
		}
	...
	public function authenticate($authCode=null) {
		try {
			...
			throw new CHttpException(500,'Unable to authenticate with Auth code.');
	...
	protected function getData($metric,$params=array())
	{
		try {
			...
			throw new CHttpException(500,'There was an Google Analytics API service error.' . $e->getCode() . $e->getMessage());
			//array('{code}'=>,'{message}'=>$e->getMessage())			));
		}
	}
		...
		if ($total>0 && count($result->rows)>0) {
			foreach ($result->rows as $item) {
				...
				if ($item[0]=='referral' && $item[1]>0) {
					$data=(object)array(
						'label'=>'Referral '. Yii::app()->numberFormatter->formatPercentage($item[1]/$total),
						'value'=>(int)$item[1],
					);
					$trafficData[]=$data;
				}
				if ($item[0]=='(none)' && $item[1]>0) {
					$data=(object)array(
						'label'=>'Direct ' . Yii::app()->numberFormatter->formatPercentage($item[1]/$total),
						'value'=>(int)$item[1],
					);
					$trafficData[]=$data;
				}
			...
			if ($searchTraffic>0) {
				$data=(object)array(
					'label'=>'Search ' . Yii::app()->numberFormatter->formatPercentage($searchTraffic/$total),
